...
model = RNNClassifier(VOCAB_SIZE, 128, 96, len(label_encoder.classes_)).to(device)
...
}).to_csv("rnn_predictions_output.csv", index=False)

pd.DataFrame(classification_report(y_true, y_preds, target_names=label_encoder.classes_, output_dict=True)).transpose().to_csv("rnn_classification_report.csv")