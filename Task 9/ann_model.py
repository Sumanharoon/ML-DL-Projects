...
model = ANNClassifier(VOCAB_SIZE, 128, 96, len(label_encoder.classes_)).to(device)
...
}).to_csv("ann_predictions_output.csv", index=False)

pd.DataFrame(classification_report(y_true, y_preds, target_names=label_encoder.classes_, output_dict=True)).transpose().to_csv("ann_classification_report.csv")